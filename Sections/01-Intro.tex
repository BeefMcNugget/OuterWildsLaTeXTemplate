...
\begin{figure}[h]
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt, draw = OWPurple, line width = 3pt] (lptop) at (-4 ,0) {\includegraphics[width = 7.8cm]{images/lptop.jpg}};
        \node[inner sep=0pt, draw = OWBlue, line width = 7pt, rotate = 35] (mstop) at (3.5 ,0) {\includegraphics[width = 9cm]{images/mstop.jpg}};
    \end{tikzpicture}
...
\begin{table}[h]
    ...
    % \renewcommand{\arraystretch}{1.3} % tblr does this internally!
    %  Orange line colors now set globally in preamble
    \begin{tblr}{ % use tblr environment for simpler table customization
         width = .5\linewidth,
         colspec = {X[1] c},
         stretch = 1.3,
         row{1} = {fg=OWOrange, font=\bfseries} % set first line to all boldface and orange
         }
         \toprule
        Heading 1 & Heading Number Two \\
         \midrule
         Stuff & Numbers \\
         Stuff & Numbers \\
         Stuff & Numbers \\
